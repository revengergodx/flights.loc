...
@section('content')
    <div class="max-w-2xl mx-auto p-6 bg-white border border-gray-200 rounded-lg shadow-lg">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">Flight edit</h2>
        ...
        <form action="{{ route('flights.update', $flight->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="airline" name="airline" value="{{ $flight->airline }}"
                       class="mt-1 block w-full p-2 border border-gray-300 rounded-md" required>
            </div>
            <div class="mb-4">
                <label for="flight_number" class="block text-sm font-medium text-gray-700">Flight Number</label>
                <input type="text" id="flight_number" name="flight_number" value="{{ $flight->flight_number }}"
                       class="mt-1 block w-full p-2 border border-gray-300 rounded-md" required>
                @error('flight_number')
            ...
            <div class="mb-4">
                ...
                <select id="departure_airport_id" name="departure_airport_id"
                    ...
                    @foreach($airports as $airport)
                        <option
                            value="{{ $airport->id }}" @selected($flight->departureAirport->id == $airport->id)>
            ...
            <div class="mb-4">
                ...
                <select id="arrival_airport_id" name="arrival_airport_id"
                    ...
                    @foreach($airports as $airport)
                        <option
                            value="{{ $airport->id }}" @selected($flight->arrivalAirport->id == $airport->id)>
                            {{ $airport->name }}, {{ $airport->city->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                ...
                <div class="relative">
                    <input type="datetime-local" id="departure_time" name="departure_time" value="{{ $flight->departure_time }}"
                           class="mt-1 block w-full p-2 border border-gray-300 rounded-md" required
            ...
            <div class="mb-4">
                ...
                <div class="relative">
                    <input type="datetime-local" id="arrival_time" name="arrival_time" value="{{ $flight->arrival_time }}"
                           class="mt-1 block w-full p-2 border border-gray-300 rounded-md" required>
                    @error('arrival_time')
                    <div class="text-red-700">{{ $message }}</div>
                    @enderror
            ...
            <div class="mb-4">
                ...
                <input type="number" id="total_seats" name="total_seats" value="{{ $flight->total_seats }}"
                       class="mt-1 block w-full p-2 border border-gray-300 rounded-md" required>
            </div>
            <div class="mb-4">
                <label for="available_seats" class="block text-sm font-medium text-gray-700">Available Seats</label>
                <input type="number" id="available_seats" name="available_seats" value="{{ $flight->available_seats }}"
                       class="mt-1 block w-full p-2 border border-gray-300 rounded-md" required>
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" id="price" name="price" value="{{ $flight->price }}"
                       class="mt-1 block w-full p-2 border border-gray-300 rounded-md" required>
            </div>
    ...
    <script>
        ...
        window.addEventListener('load', updateArrivalMinDate);
